...
export interface DeliveryStopStatusCounts {
  ...
  pending: number;
  inProgress: number;
  // Fase 99 -- FAILED (entrega tentada mas nao concluida) e um novo status
  // terminal, distinto de CANCELLED (removida antes de qualquer tentativa).
  // Contado separadamente para nunca ser perdido do total (ver `total` em
  // classifyEmptyTripReason abaixo).
  failed: number;
}
...
// ALL_DELIVERIES_CANCELLED: havia paradas planejadas, mas nenhuma foi
//   concluida com sucesso -- todas CANCELLED e/ou FAILED (Fase 99: uma
//   entrega tentada e sem sucesso conta aqui do mesmo jeito que uma
//   removida do planejamento -- em ambos os casos a viagem saiu vazia sem
//   nenhuma entrega efetivada).
// DELIVERIES_INCOMPLETE: havia paradas planejadas, nenhuma concluida, mas
//   nem todas canceladas/com falha (ex: ainda PENDING/IN_PROGRESS) --
//   situacao real mas sem motivo definitivo (a viagem terminou antes de
//   resolver as paradas restantes).
...
export function classifyEmptyTripReason(counts: DeliveryStopStatusCounts): EmptyTripReason {
  const total = counts.completed + counts.cancelled + counts.pending + counts.inProgress + counts.failed;
  if (total === 0) return 'NO_DELIVERIES_PLANNED';
  if (counts.completed > 0) return 'COMPLETED_DELIVERIES_INCONSISTENT';
  if (counts.cancelled + counts.failed === total) return 'ALL_DELIVERIES_CANCELLED';
