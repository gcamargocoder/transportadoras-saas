...
// BI 5 -- mesmo padrao de FleetVehicleTable (BI 4): cada KPI de custo tem
// sua propria chamada de breakdown?dimension=vehicle (mesmo endpoint do
// custo por veiculo). limit alto: tabela investigativa (todos os veiculos),
// nao um "top N".
...
const METRICS: MetricConfig[] = [
  { kpiId: 'operating_cost', unit: 'BRL', header: 'Custo total' },
  { kpiId: 'fuel_cost', unit: 'BRL', header: 'Combustível' },
  { kpiId: 'maintenance_cost', unit: 'BRL', header: 'Manutenção' },
  { kpiId: 'toll_cost', unit: 'BRL', header: 'Pedágio' },
  { kpiId: 'tire_cost', unit: 'BRL', header: 'Pneus' },
  { kpiId: 'other_cost', unit: 'BRL', header: 'Outras despesas' },
  { kpiId: 'cost_per_km', unit: 'BRL_PER_KM', header: 'Custo/km' },
];
...
// BI 5 -- tabela investigativa de custo por veiculo: ordenacao e busca sao
// ferramentas do usuario, nunca um ranking do sistema (sem rotulo de
// "melhor"/"pior veiculo"). Uma linha por veiculo que aparece em QUALQUER um
// dos 7 breakdowns; celula sem dado mostra "-" + motivo, nunca 0 inventado
// (0 real -- veiculo sem gasto na categoria -- aparece como "R$ 0,00").
export function CostVehicleTable({ range, fleetId }: { range: PeriodRange; fleetId: string | null }): JSX.Element {
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebounce(search);
  const [sortBy, setSortBy] = useState<string>('operating_cost');
